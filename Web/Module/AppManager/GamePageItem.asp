...
<body>
    ...
    <% 
        ...
        Sub SavePageInfo()
            '判断重复
            Dim ret
            Set ret = GameConn.execute("select * from GamePageItem where PageID<>'"&Request("id")&"' AND PageID='"&CxGame.GetInfo(1,"in_PageID")&"'")
            If Not ret.eof Then
                Call CxGame.ShowInfo2("页面标识重复，操作失败！","GamePageItem.asp",1200)
                Exit Sub
            End If
