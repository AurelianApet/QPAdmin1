...
<body>
    ...
    <% 
        ...
        Sub SaveTypeInfo()
            '判断重复
            Dim ret
            Set ret = GameConn.execute("select * from GameTypeItem where TypeID<>'"&Request("id")&"' AND TypeID='"&CxGame.GetInfo(1,"in_TypeID")&"'")
            If Not ret.eof Then
                Call CxGame.ShowInfo2("类型标识重复，操作失败！","GameTypeItem.asp",1200)
                Exit Sub
            End If
