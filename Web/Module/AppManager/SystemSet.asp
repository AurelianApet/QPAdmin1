...
<body>
    ...
    <% 
        ...
        Sub Save()          
            ...
            Set rs=Server.CreateObject("Adodb.RecordSet")
            sql="select  * from dbo.SystemStatusInfo where StatusName='RevenueRateTake'"
            rs.open sql,GameConn,1,3
            If rs.eof then 
                rs.addnew      
                rs("StatusName") = "RevenueRateTake"
            End If  
            rs("StatusValue") = CxGame.GetInfo(1,"in_RevenueRateTake")
            rs("StatusString") = CxGame.GetInfo(0,"in_RevenueRateTakeNote")
            rs.update
            
            Set rs=Server.CreateObject("Adodb.RecordSet")
            sql="select  * from dbo.SystemStatusInfo where StatusName='RevenueRateTransfer'"
            rs.open sql,GameConn,1,3
            If rs.eof then 
                rs.addnew      
                rs("StatusName") = "RevenueRateTransfer"
            End If  
            rs("StatusValue") = CxGame.GetInfo(1,"in_RevenueRateTransfer")
            rs("StatusString") = CxGame.GetInfo(0,"in_RevenueRateTransferNote")
            rs.update
            
            Set rs=Server.CreateObject("Adodb.RecordSet")
            sql="select  * from dbo.SystemStatusInfo where StatusName='BankPrerequisite'"
            rs.open sql,GameConn,1,3
            If rs.eof then 
                rs.addnew      
                rs("StatusName") = "BankPrerequisite"
            End If  
            rs("StatusValue") = CxGame.GetInfo(1,"in_BankPrerequisite")
            rs("StatusString") = CxGame.GetInfo(0,"in_BankPrerequisiteNote")
            rs.update
            
            Set rs=Server.CreateObject("Adodb.RecordSet")
            sql="select  * from dbo.SystemStatusInfo where StatusName='TransferPrerequisite'"
            rs.open sql,GameConn,1,3
            If rs.eof then 
                rs.addnew      
                rs("StatusName") = "TransferPrerequisite"
            End If  
            rs("StatusValue") = CxGame.GetInfo(1,"in_TransferPrerequisite")
            rs("StatusString") = CxGame.GetInfo(0,"in_TransferPrerequisiteNote")
            rs.update
        ...
        Sub Main()
            ...
            Dim GrantMachineCount,GrantMachineCountNote,GrantScoreCount,GrantScoreCountNote,MedalRate,MedalRateNote,RevenueRateTake,RevenueRateTakeNote,RevenueRateTransfer,RevenueRateTransferNote
            Dim TransferPrerequisite,TransferPrerequisiteNote
            Dim BankPrerequisite,BankPrerequisiteNote
            Set rs=Server.CreateObject("Adodb.RecordSet")
            sql="select  * from dbo.SystemStatusInfo where StatusName='EnjoinInsure'"
            rs.open sql,GameConn,1,3
            if  rs.eof  then  
            ...
            if  rs.eof  then  
                RevenueRateTake = "0"
                RevenueRateTakeNote="银行存取操作税收比率（千分比）！"
            Else
                RevenueRateTake = rs("StatusValue")
                RevenueRateTakeNote=rs("StatusString")
            End If 
            ...
            rs.open sql,GameConn,1,3
            if  rs.eof  then  
                RevenueRateTransfer = "0"
                RevenueRateTransferNote="银行转账操作税收比率（千分比）！"
            Else
                RevenueRateTransfer = rs("StatusValue")
                RevenueRateTransferNote=rs("StatusString")
            End If 
            ...
            if  rs.eof  then  
                BankPrerequisite = "0"
                BankPrerequisiteNote="银行存取条件，存取金币数必须大于此数才可操作！"
            Else
                BankPrerequisite = rs("StatusValue")
                BankPrerequisiteNote=rs("StatusString")
            End If 
            ...
            rs.open sql,GameConn,1,3
            if  rs.eof  then  
                TransferPrerequisite = "0"
                TransferPrerequisiteNote="银行转账条件，银行金币数必须大于此数才可转账！"
            Else
                TransferPrerequisite = rs("StatusValue")
                TransferPrerequisiteNote=rs("StatusString")
            End If 
    ...
    <form name="myFormInfo2" method="post" action='?action=save' onsubmit="return CheckFormInfo()">
        ...
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listBg2">
            ...
            <tr>
                <td height="35" colspan="2" class="f14 bold Lpd10 Rpd10"><div class="hg3  pd7">银行取款税收比率</div></td>
            </tr>
            <tr>
                <td class="listTdLeft">数目：</td>
                <td>
                    <input name="in_RevenueRateTake" type="text" class="text" style="width:300px;" value="<%=RevenueRateTake %>" />‰                      
                </td>
            </tr>
            <tr>
                <td class="listTdLeft">说明：</td>
                <td>
                    <input name="in_RevenueRateTakeNote" type="text" class="text" style="width:500px;" value="<%=RevenueRateTakeNote %>" />  
                </td>
            </tr>
            <tr>
                <td height="35" colspan="2" class="f14 bold Lpd10 Rpd10"><div class="hg3  pd7">银行转账税收比率</div></td>
            </tr>
            <tr>
                <td class="listTdLeft">数目：</td>
                <td>
                    <input name="in_RevenueRateTransfer" type="text" class="text" style="width:300px;" value="<%=RevenueRateTransfer %>" />‰                      
                </td>
            </tr>
            <tr>
                <td class="listTdLeft">说明：</td>
                <td>
                    <input name="in_RevenueRateTransferNote" type="text" class="text" style="width:500px;" value="<%=RevenueRateTransferNote %>" />  
                </td>
            </tr>
            <tr>
                <td height="35" colspan="2" class="f14 bold Lpd10 Rpd10"><div class="hg3  pd7">银行存取限制</div></td>
            </tr>
            <tr>
                <td class="listTdLeft">数目：</td>
                <td>
                    <input name="in_BankPrerequisite" type="text" class="text" style="width:300px;" value="<%=BankPrerequisite %>" />                     
                </td>
            </tr>
            <tr>
                <td class="listTdLeft">说明：</td>
                <td>
                    <input name="in_BankPrerequisiteNote" type="text" class="text" style="width:500px;" value="<%=BankPrerequisiteNote %>" />  
                </td>
            </tr>
            <tr>
                <td height="35" colspan="2" class="f14 bold Lpd10 Rpd10"><div class="hg3  pd7">银行转账限制</div></td>
            </tr>
            <tr>
                <td class="listTdLeft">数目：</td>
                <td>
                    <input name="in_TransferPrerequisite" type="text" class="text" style="width:300px;" value="<%=TransferPrerequisite %>" />                     
                </td>
            </tr>
            <tr>
                <td class="listTdLeft">说明：</td>
                <td>
                    <input name="in_TransferPrerequisiteNote" type="text" class="text" style="width:500px;" value="<%=TransferPrerequisiteNote %>" />  
                </td>
            </tr>
            <tr>
