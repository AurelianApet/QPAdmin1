...
<body>
    ...
    <% 
        ...
        Sub SaveKindInfo()
            '判断重复
            Dim ret
            Set ret = GameConn.execute("select * from GameKindItem where KindID<>'"&Request("id")&"' AND KindID='"&CxGame.GetInfo(1,"in_KindID")&"'")
            If Not ret.eof Then
                Call CxGame.ShowInfo2("游戏标识重复，操作失败！","GameKindItem.asp",1200)
                Exit Sub
            End If
