...
<body>
    ...
    <% 
        ...
        Sub SaveGameInfo()
            '判断重复
            Dim ret
            Set ret = GameConn.execute("select * from GameGameItem where GameID<>'"&Request("id")&"' AND GameID='"&CxGame.GetInfo(1,"in_GameID")&"'")
            If Not ret.eof Then
                Call CxGame.ShowInfo2("模块标识重复，操作失败！","GameGameItem.asp",1200)
                Exit Sub
            End If
