...
<body>
    ...
    <!-- 头部菜单 End -->  
     <%
        ...
        Sub DeleteInfo(lID)
            Dim ID
            ID = Trim(lID)
            GameConn.execute "Delete From MatchInfo Where MatchID="&ID
	        Call CxGame.ShowInfo2("删除成功！","MatchList.asp",1200)
        End Sub
        ...
        Sub Nullity(lID,typeValue)
            Dim ID
            ID = lID
            GameConn.execute "Update MatchInfo Set Nullity="&typeValue&" Where MatchID="&lID
            If typeValue=1 Then
                Call CxGame.ShowInfo2("冻结成功！","MatchList.asp",1200)
            Else
                Call CxGame.ShowInfo2("解冻成功！","MatchList.asp",1200)
            End If
        End Sub
	    ...
	    Sub Save()
            ...
            sql = "select * from MatchInfo where MatchID='"&Request("MatchID")&"'"
            rs.Open sql,GameConn,1,3
            If rs.eof Then
                rs.addnew
            End If
            rs("MatchName") = Request("MatchName")
            rs("MatchSummary") = Request("MatchSummary")
            rs("MatchDate") = Request("MatchDate")
            rs("MatchContent") = Request("content1")
            rs("MatchStatus") = Request("MatchStatus")
            rs("Nullity") = Request("Nullity")
            rs.update
        ...
        <div id="content">        
            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="box">
                <tr align="center" class="bold">
                     <td class="listTitle1" style="width:30px;"><input type="checkbox" name="chkAll" onclick="SelectAll(this.checked);" /></td>
                    <th class="listTitle1"><a class="l1"  href="" onclick="GetOrder('MatchID',this)">比赛标识</a></th>
                    <th class="listTitle2"><a class="l1"  href="" onclick="GetOrder('MatchName',this)">比赛名称</a></th>
                    <th class="listTitle2"><a class="l1"  href="" onclick="GetOrder('MatchSummary',this)">比赛描述</a></th>
                    <th class="listTitle2"><a class="l1"  href="" onclick="GetOrder('MatchDate',this)">比赛时间</a></th>
                    <th class="listTitle2"><a class="l1"  href="" onclick="GetOrder('MatchStatus',this)">赛事状态</a></th>
                ...
                <tr class="<%=className %>" onmouseover="currentcolor=this.style.backgroundColor;this.style.backgroundColor='#caebfc';this.style.cursor='pointer';"
                    ...
                    <td onclick="Redirect('MatchList.asp?action=newinfo&MatchID=<%=rs(0,i) %>')"><%=rs(2,i) %></td>    
                    <td onclick="Redirect('MatchList.asp?action=newinfo&MatchID=<%=rs(0,i) %>')"><%=rs(3,i) %></td>          
                    <td>
                        <% If rs(5,i)=0 Then%>
                        未开始                        
                        <% 
                    ...
                    <td onclick="Redirect('MatchList.asp?action=newinfo&MatchID=<%=rs(0,i) %>')"><%=rs(7,i) %></td> 
                </tr>
                <% 
                    Next
                    End If
    ...
    <%
        ...
        Sub NewInfo()
            Dim rs,sql,MatchID,MatchName,MatchSummary,MatchDate,MatchContent,MatchStatus,Nullity
    ...
    <script type="text/javascript">
		...
		{
		    var MatchName = document.form1.MatchName.value.trim();
		    var MatchSummary = document.form1.MatchSummary.value.trim();
		    var MatchDate = document.form1.MatchDate.value.trim();
    ...
    <form name="form1" method="post" action="?action=save&MatchID=<%=MatchID %>"   onsubmit="return CheckFormInfo();">
         ...
         <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listBg2">
            ...
            <tr>
                ...
                <td>
                    <input name="MatchName" type="text" class="text wd4" style="width:694px" value="<%=MatchName %>" />
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <td class="listTdLeft">比赛时间：</td>
                <td>
                    <input name="MatchDate"  type="text" class="text wd4" style="width:694px" value="<%=MatchDate %>" />
                </td>
            </tr>
            <tr>
                <td class="listTdLeft">比赛状态：</td>
                <td>
                    <input name="MatchStatus" type="radio"  value="0" <%If MatchStatus=0 Then %> checked="checked" <%End If %> />未开始
                    <input name="MatchStatus"  type="radio" value="1" <%If MatchStatus=1 Then %> checked="checked" <%End If %> />进行中
                    <input name="MatchStatus"  type="radio" value="2" <%If MatchStatus=2 Then %> checked="checked" <%End If %> />已结束
                </td>
            </tr>
            <tr>
