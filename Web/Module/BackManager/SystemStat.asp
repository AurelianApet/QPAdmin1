...
<body>
    ...
    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listBg2">
        ...
        <tr>
            <td height="35" colspan="2" class="f14 bold Lpd10 Rpd10"><div class="hg3  pd7">赠送统计</div></td>
        </tr>
        <tr>
            <td class="listTdLeft">注册赠送：</td>
            <td>
                <% 
                    Call ConnectGame(QPAccountsDB)
                        Set rs = GameConn.Execute("select sum(GrantScore) from SystemGrantCount(nolock)")
                        If IsNull(rs(0)) Then
        ...
        <tr>
            <td class="listTdLeft">泡分赠送：</td>
            <td>
                <% 
                    Call ConnectGame(QPTreasureDB)
                        Set rs = GameConn.Execute("select sum(PresentScore) from StreamPlayPresent(nolock)")
                        If IsNull(rs(0)) Then
                            response.Write FormatNumber(0,0,-1)
                        Else
                            response.Write FormatNumber(rs(0),0,-1)
        ...
        <tr>
            <td class="listTdLeft">已兑换金币量：</td>
            <td>
                 <% 
                    Call ConnectGame(QPRecordDB)
                        Set rs = GameConn.Execute("select sum(ConvertPresent*ConvertRate) from RecordConvertPresent(nolock)")
                        If IsNull(rs(0)) Then
                            response.Write FormatNumber(0,0,-1)
                        Else
                            response.Write FormatNumber(rs(0),0,-1)
        ...
        <tr>
            ...
            <td>
                <div class="gamelist">                    
                    <% 
                        Call ConnectGame(QPTreasureDB)
                            Set rs = GameConn.Execute("select KindID,Sum(Revenue) from RecordDrawInfo(nolock) group by KindID")
        ...
        <tr style="padding-top:20px">
            ...
            <td>
                <div class="gamelist">                    
                    <% 
                        Call ConnectGame(QPTreasureDB)
                            Set rs = GameConn.Execute("select ServerID,Sum(Revenue) from RecordDrawInfo(nolock) group by ServerID")
                            If Not IsNull(rs) Then                                
                            do until rs.eof
        ...
        <tr>
            <td height="35" colspan="2" class="f14 bold Lpd10 Rpd10"><div class="hg3  pd7">损耗统计</div></td>
        </tr>
        <tr>
            <td class="listTdLeft">损耗总量：</td>
            <td>
                <% 
                    Call ConnectGame(QPTreasureDB)
                        Set rs = GameConn.Execute("select sum(Waste) from RecordDrawInfo(nolock)")
                        If IsNull(rs(0)) Then
                            response.Write FormatNumber(0,0,-1)
                        Else
                            response.Write FormatNumber(rs(0),0,-1)
        ...
        <tr>
            <td class="listTdLeft">游戏损耗：</td>
            <td>
                <div class="gamelist">                    
                    <% 
                        Call ConnectGame(QPTreasureDB)
                            Set rs = GameConn.Execute("select KindID,Sum(Waste) from RecordDrawInfo(nolock) group by KindID")
                            If Not IsNull(rs) Then                                 
                            do until rs.eof
                            response.Write  "<span>"&CxGame.GetKindNameByKindID(rs(0))&"("&FormatNumber(rs(1),0,-1)&")</span>"    
                            rs.movenext
        ...
        <tr style="padding-top:20px">
            <td class="listTdLeft">房间损耗：</td>
            <td>
                <div class="gamelist">                    
                    <% 
                        Call ConnectGame(QPTreasureDB)
                            Set rs = GameConn.Execute("select ServerID,Sum(Waste) from RecordDrawInfo(nolock) group by ServerID")
                            If Not IsNull(rs) Then                                
                            do until rs.eof
                            response.Write  "<span>"&CxGame.GetRoomNameByRoomID(rs(0))&"("&FormatNumber(rs(1),0,-1)&")</span>"    
                            rs.movenext
