...
<body>
    ...
    <% 
        ...
        Sub InsertRecord()
            ...
            '新增帐号修改记录
            If CxGame.GetInfo(0,"in_Nickname") <> CxGame.GetInfo(0,"hidNickName") Then
                Dim sql_rUser
                sql_rUser = "insert into QPRecordDBLink.QPRecordDB.dbo.RecordAccountsExpend(OperMasterID,UserID,ReAccounts,ClientIP) values("
                sql_rUser = sql_rUser&masterID&","&Request("id")&",'"&CxGame.GetInfo(0,"in_Nickname")&"','"&clientIP&"')"
        ...
        Sub SaveUserInfo()
            ...
            If rs1.eof and rs2.eof Then
                ...
                rs("IsAndroid") = CxGame.GetInfo(1,"in_IsAndroid")
    ...
    <form name="form1" method="post" action='?action=save&id=<%=Request("id") %>' onsubmit="return CheckForm()">
        ...
        <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0" class="listBg2">
	        ...
	        <tr>
	            ...
	            <td>
	                <input name="hidNickName" type="hidden" value="<%=rs("NickName") %>"/>
	        ...
	        <tr>
	            <td class="listTdLeft">密保卡：</td>
	            <td>
	                <% If rs("PasswordID")=0 Then%>
                    <span style="font-weight:bold;">未申请</span>
                    <% Else%>
                    <span style="font-weight:bold;">已申请</span>&nbsp;<a href="javascript:openWindow('AccountsPassword.asp?UserID=<%=Request("id") %>',550,320)" class="l1">点击查看</a>
                    <% End If %>                
	            </td>
	        </tr>
	        <tr>
	        ...
	        <tr>
	            ...
	            <td>
	                <input name="in_UnderWrite" type="text" class="text wd4" style="width:500px" value="<%=rs("UnderWrite") %>"/>&nbsp;长度不可超过63个字符    
	            </td>
	        </tr>	        
	        <tr>
	        ...
	        <tr>
	            <td class="listTdLeft">机器人：</td>
	            <td>
	                <input name="in_IsAndroid" id="in_IsAndroid" type="checkbox" value="1"<% If rs("IsAndroid")=1 Then %> checked="checked"<% End If %> /><label for="in_IsAndroid">机器人标识</label>
	            </td>
	        </tr>
	        <tr>
