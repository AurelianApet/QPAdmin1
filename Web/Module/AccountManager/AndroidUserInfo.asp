...
<body>
    ...
    <% 
        ...
        Sub SaveAndroidInfo()
            ...
            rs("MinTakeScore") = CxGame.GetInfo(0,"MinTakeScore")
            If CxGame.GetInfo(0,"MinTakeScore")="" Then
                rs("MinTakeScore")=0
            End If
            rs("MaxTakeScore") = CxGame.GetInfo(0,"MaxTakeScore")
            If CxGame.GetInfo(0,"MaxTakeScore")="" Then
                rs("MaxTakeScore")=0
            End If
            ...
            Next
            rs("ServiceGender") = ServiceGender
            '服务时间
        ...
        Sub Main()
            ...
            Dim field
            field = "CreateDate,UserID,ServerID,AndroidNote,Nullity,ServiceGender"
            '查询条件            
        ...
        <div id="content">
            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="box">
                <tr align="center" class="bold">
                    ...
                    <th class="listTitle2"><a  class="l1" href="" onclick="GetOrder('UserID',this)">用户帐号(GameID)</a></th>
                    <% If dname="QPTreasureDB" Then %>
                    <th class="listTitle2"><a  class="l1" href="" onclick="GetOrder('UserID',this)">金币+银行</a></th>
                    <% Else %>
                    <th class="listTitle2"><a  class="l1" href="" onclick="GetOrder('UserID',this)">积分</a></th>
                    <% End If %>   
                    ...
                    <th class="listTitle2"><a class="l1"  href="" onclick="GetOrder('ServiceGender',this)">机器人类型</a></th>
                ...
                <tr class="<%=className %>" onmouseover="currentcolor=this.style.backgroundColor;this.style.backgroundColor='#caebfc';this.style.cursor='pointer';"
                    ...
                    <td onclick="Redirect('AndroidUserInfo.asp?action=androidInfo&UserID=<%=rs(1,i) %>&ServerID=<%=rs(2,i) %>&dbName=<%=dname %>&page=<%=Request("page") %>')"><%=rs(0,i) %></td>   
                    <td>
                        <a class="l" href="javascript:void(0)" onclick="javascript:openWindowOwn('AccountsInfo.asp?id=<%=rs(1,i) %>',<%=rs(1,i) %>,850,600)"><%=CxGame.GetUserNameByUserID(rs(1,i))&"("&CxGame.GetGameIDByUserID(rs(1,i))&")" %></a>
                    </td>
                    <td onclick="Redirect('AndroidUserInfo.asp?action=androidInfo&UserID=<%=rs(1,i) %>&ServerID=<%=rs(2,i) %>&dbName=<%=dname %>&page=<%=Request("page") %>')"><%=CxGame.GetScoreInfoByUserID(rs(1,i),dname)(1,0) %></td>  
                    <td onclick="Redirect('AndroidUserInfo.asp?action=androidInfo&UserID=<%=rs(1,i) %>&ServerID=<%=rs(2,i) %>&dbName=<%=dname %>&page=<%=Request("page") %>')"><%=CxGame.GetScoreInfoByUserID(rs(1,i),dname)(1,1) %></td>  
                    <td onclick="Redirect('AndroidUserInfo.asp?action=androidInfo&UserID=<%=rs(1,i) %>&ServerID=<%=rs(2,i) %>&dbName=<%=dname %>&page=<%=Request("page") %>')"><%=CxGame.GetScoreInfoByUserID(rs(1,i),dname)(1,2) %></td>  
                    <td onclick="Redirect('AndroidUserInfo.asp?action=androidInfo&UserID=<%=rs(1,i) %>&ServerID=<%=rs(2,i) %>&dbName=<%=dname %>&page=<%=Request("page") %>')"><%=CxGame.GetRoomNameByRoomID(rs(2,i)) %></td> 
                    <td onclick="Redirect('AndroidUserInfo.asp?action=androidInfo&UserID=<%=rs(1,i) %>&ServerID=<%=rs(2,i) %>&dbName=<%=dname %>&page=<%=Request("page") %>')">
                        <% If rs(5,i)=0 Then %>
                        挂线型
                        <% Elseif rs(5,i)=1 Then  %>
                        相互模拟型
                        <% Elseif rs(5,i)=2 Then  %>
                        被动陪打型
                        <% Elseif rs(5,i)=4 Then  %>
                        主动陪打型
                        <% Elseif rs(5,i)=3 Then  %>
                        相互模拟型，被动陪打型
                        <% Elseif rs(5,i)=5 Then  %>
                        相互模拟型，主动陪打型
                        <% Elseif rs(5,i)=6 Then  %>
                        被动陪打型，主动陪打型
                        <% Elseif rs(5,i)=7 Then  %>
                        相互模拟型，被动陪打型，主动陪打型
                        <% Else %>
                        未知
                        <% End If %>
                    </td>   
                    <td onclick="Redirect('AndroidUserInfo.asp?action=androidInfo&UserID=<%=rs(1,i) %>&ServerID=<%=rs(2,i) %>&dbName=<%=dname %>&page=<%=Request("page") %>')"><%=rs(3,i) %></td>  
                    <td onclick="Redirect('AndroidUserInfo.asp?action=androidInfo&UserID=<%=rs(1,i) %>&ServerID=<%=rs(2,i) %>&dbName=<%=dname %>&page=<%=Request("page") %>')"><%=CxGame.GetNullityInfo(rs(4,i)) %></td>                      
                </tr>
                <% 
                    Next  
                    End If    
    ...
    <% 
        ...
        Sub GetAndroidInfo
            ...
            Dim rs,sql
            Dim UserID,ServerID,MinPlayDraw,MaxPlayDraw,MinTakeScore,MaxTakeScore,MinReposeTime,MaxReposeTime,ServiceTime,ServiceGender,AndroidNote,Nullity,CreateDate                
            Dim xorObj            
            Set rs=Server.CreateObject("Adodb.RecordSet")
            ...
            Call CloseGame()
    %>
    <form name="myFormInfo" method="post" action='?action=saveandroidinfo&UserID=<%=Request("UserID") %>&ServerID=<%=Request("ServerID") %>&dbName=<%=Request("dbName") %>&page=<%=Request("page") %>'>
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="Tmg7">
            <tr>
        ...
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listBg2">
            ...
            <tr>
                ...
                <td> 
                    <input name="ServiceGender" id="chkServiceGender1" type="checkbox" value="1"<% If ServiceGender And 1 Then %> checked="checked" <%End If %> /><label for="chkServiceGender1">相互模拟</label> 
                    <input name="ServiceGender" id="chkServiceGender2" type="checkbox" value="2"<% If ServiceGender And 2 Then %> checked="checked" <%End If %> /><label for="chkServiceGender2">被动陪打</label>            
                    <input name="ServiceGender" id="chkServiceGender3" type="checkbox" value="4"<% If ServiceGender And 4 Then %> checked="checked" <%End If %> /><label for="chkServiceGender3">主动陪打</label>      
            ...
            <tr>
                ...
                <td> 
                    ...
                    <tr>
                        <td><input name="ServiceTime" id="chkServiceTime1" type="checkbox" value="1"<% If ServiceTime And 1 Then %> checked="checked" <%End If %> /><label for="chkServiceTime1">0:00-1:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime2" type="checkbox" value="2"<% If ServiceTime And 2 Then %> checked="checked" <%End If %> /><label for="chkServiceTime2">1:00-2:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime3" type="checkbox" value="4"<% If ServiceTime And 4 Then %> checked="checked" <%End If %> /><label for="chkServiceTime3">2:00-3:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime4" type="checkbox" value="8"<% If ServiceTime And 8 Then %> checked="checked" <%End If %> /><label for="chkServiceTime4">3:00-4:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime5" type="checkbox" value="16"<% If ServiceTime And 16 Then %> checked="checked" <%End If %> /><label for="chkServiceTime5">4:00-5:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime6" type="checkbox" value="32"<% If ServiceTime And 32 Then %> checked="checked" <%End If %> /><label for="chkServiceTime6">5:00-6:00</label></td>
                    </tr>
                    <tr>
                        <td><input name="ServiceTime" id="chkServiceTime7" type="checkbox" value="64"<% If ServiceTime And 64 Then %> checked="checked" <%End If %> /><label for="chkServiceTime7">6:00-7:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime8" type="checkbox" value="128"<% If ServiceTime And 128 Then %> checked="checked" <%End If %> /><label for="chkServiceTime8">7:00-8:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime9" type="checkbox" value="256"<% If ServiceTime And 256 Then %> checked="checked" <%End If %> /><label for="chkServiceTime9">8:00-9:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime10" type="checkbox" value="512"<% If ServiceTime And 512 Then %> checked="checked" <%End If %> /><label for="chkServiceTime10">9:00-10:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime11" type="checkbox" value="1024"<% If ServiceTime And 1024 Then %> checked="checked" <%End If %> /><label for="chkServiceTime11">10:00-11:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime12" type="checkbox" value="2048"<% If ServiceTime And 2048 Then %> checked="checked" <%End If %> /><label for="chkServiceTime12">11:00-12:00</label></td>
                    </tr>
                    <tr>
                        <td><input name="ServiceTime" id="chkServiceTime13" type="checkbox" value="4096"<% If ServiceTime And 4096 Then %> checked="checked" <%End If %> /><label for="chkServiceTime13">12:00-13:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime14" type="checkbox" value="8192"<% If ServiceTime And 8192 Then %> checked="checked" <%End If %> /><label for="chkServiceTime14">13:00-14:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime15" type="checkbox" value="16384"<% If ServiceTime And 16384 Then %> checked="checked" <%End If %> /><label for="chkServiceTime15">14:00-15:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime16" type="checkbox" value="32768"<% If ServiceTime And 32768 Then %> checked="checked" <%End If %> /><label for="chkServiceTime16">15:00-16:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime17" type="checkbox" value="65536"<% If ServiceTime And 65536 Then %> checked="checked" <%End If %> /><label for="chkServiceTime17">16:00-17:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime18" type="checkbox" value="131072"<% If ServiceTime And 131072 Then %> checked="checked" <%End If %> /><label for="chkServiceTime18">17:00-18:00</label></td>
                    </tr>
                    <tr>
                        <td><input name="ServiceTime" id="chkServiceTime19" type="checkbox" value="262144"<% If ServiceTime And 262144 Then %> checked="checked" <%End If %> /><label for="chkServiceTime19">18:00-19:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime20" type="checkbox" value="524288"<% If ServiceTime And 524288 Then %> checked="checked" <%End If %> /><label for="chkServiceTime20">19:00-20:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime21" type="checkbox" value="1048576"<% If ServiceTime And 1048576 Then %> checked="checked" <%End If %> /><label for="chkServiceTime21">20:00-21:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime22" type="checkbox" value="2097152"<% If ServiceTime And 2097152 Then %> checked="checked" <%End If %> /><label for="chkServiceTime22">21:00-22:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime23" type="checkbox" value="4194304"<% If ServiceTime And 4194304 Then %> checked="checked" <%End If %> /><label for="chkServiceTime23">22:00-23:00</label></td>
                        <td><input name="ServiceTime" id="chkServiceTime24" type="checkbox" value="8388608"<% If ServiceTime And 8388608 Then %> checked="checked" <%End If %> /><label for="chkServiceTime24">23:00-24:00</label></td>
                    </tr>
                    </table>  
                </td>
            </tr>
