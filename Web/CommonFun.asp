...
if AdminName="" or adminpassword="" then
    ...
    'response.Write "<script>top.document.location='/Login.asp'</script>"
end if
...
Class GameObj
    ...
    Function GetScoreInfoByUserID(UserID,dname)
        ...
        Else
        sql="SELECT Score, Wincount,LostCount FROM GameScoreInfo(nolock)  WHERE UserID="&UserID
        End if
        rs.Open sql,GameConn,1,3
        redim ArrayList(1,2)
        If Not rs.eof Then    
    ...
    Function GetScoreKindList()
        ...
        Call ConnectGame(QPPlatformDB)
        Set rs=Server.CreateObject("Adodb.RecordSet")
        sql = "SELECT a.KindID,a.KindName,b.DataBaseAddr,b.DataBaseName,a.TypeID FROM GameKindItem a,GameGameItem b WHERE a.GameID = b.GameID and b.DataBaseName<>'QPTreasureDB' ORDER BY SortID"
        rs.Open sql,GameConn,1,3
        n= rs.RecordCount-1
        redim ArrayList(n,4)
        j=0
    ...
    Function GetNickNameByUserID(userID)
        ...
        Call ConnectGame(QPAccountsDB)
        Set rs=Server.CreateObject("Adodb.RecordSet")
        sql="SELECT NickName FROM AccountsInfo(nolock)  WHERE UserID="&userID
        rs.Open sql,GameConn,1,3
        If Not rs.eof Then    
        ...
        set rs=nothing
        CloseGame()
        GetNickNameByUserID = NickName
    End Function 
    
    Rem 根据用户ID获取昵称
    Function GetUserMedalByUserID(userID)
        Dim rs,sql,UserMedal
        UserMedal=0
        Call ConnectGame(QPAccountsDB)
        Set rs=Server.CreateObject("Adodb.RecordSet")
        sql="SELECT UserMedal FROM AccountsInfo(nolock)  WHERE UserID="&userID
        rs.Open sql,GameConn,1,3
        If Not rs.eof Then    
            UserMedal=rs("UserMedal")
        End If 
        rs.close
        set rs=nothing
        CloseGame()
        GetUserMedalByUserID = UserMedal
    End Function 
    
    Rem 状态显示
    Function GetUserAndroidStatus(userID)
        Dim rs,sql,IsAndroid,rInfo
        IsAndroid=0
        Call ConnectGame(QPAccountsDB)
        Set rs=Server.CreateObject("Adodb.RecordSet")
        sql="SELECT IsAndroid FROM AccountsInfo(nolock)  WHERE UserID="&userID
        rs.Open sql,GameConn,1,3
        If Not rs.eof Then    
            IsAndroid=rs("IsAndroid")
        End If         
        If IsAndroid=0 Then
            rInfo = "<span>否</span>"
        Else
